...
def make_cut_header(
    n_clusters: int = 31, n_channels: int = 4, n_params: int = 2
) -> dict:
    """
    ...
    -------
    dict
        Dictionary of the cut file header
    """
    cut_header = [
...
def make_cluster_cut_entries(
    ...
    -------
    str
        String of the cluster entries for the cut file
    """
...
class EEGHeader(LFPHeader):
    ...
    lfp_entries: dict = field(default_factory=make_eeg_entries)
...
class EGFHeader(LFPHeader):
    ...
    lfp_entries: dict = field(default_factory=make_egf_entries)
...
class TetrodeHeader(AxonaHeader):
    ...
    tetrode_entries: dict = field(default_factory=make_tetrode_entries)
...
# append 1-3 to these
entries_to_number_to_three = [("BPFrecord", "0"), ("BPFbit", "0"), ("BPFEEGin", "0")]
...
# append 1-9
entries_to_number_to_nine = [("slot_chan_", "0")]

# replace X with 1-17 and Y with 0-9
entries_groups = [("groups_X_Y", "0")]
...
def make_set_entries():
    ...
    _entries_to_number_one_indexed = dict(
        [
            (e[0] + str(n), e[1])
            for e in entries_to_number_one_indexed
            for n in range(1, 65)
        ]
    )
    ...
    _entries_to_number_to_sixteen = dict(
        [
            (e[0] + str(n), e[1])
            for e in entries_to_number_to_sixteen
            for n in range(1, 17)
        ]
    )

    _entries_to_number_to_three = dict(
        [(e[0] + str(n), e[1]) for e in entries_to_number_to_three for n in range(1, 4)]
    )

    _entries_to_replace_one = dict(
        [
            (e[0].replace("1", str(n)), e[1])
            for e in entries_to_replace_one
            for n in range(1, 5)
        ]
    )

    _entries_to_number_to_nine = dict(
        [(e[0] + str(n), e[1]) for e in entries_to_number_to_nine for n in range(0, 10)]
    )

    _entries_groups = dict(
        [
            (e[0].replace("X", str(n)).replace("Y", str(m)), e[1])
            for e in entries_groups
            for n in range(1, 18)
            for m in range(10)
        ]
    )
    ...
    return {
        ...
        **_entries_to_number_to_nine,
        **_entries_to_number_to_sixteen,
        **_entries_to_number_to_three,
        **_entries_to_replace_one,
        **_entries_groups,
    }
