...
class OE2Axona(object):
    ...
    def getOEData(
        ...
        if os.path.isfile(filename_root):
            ...
            OE_data.load(
                pname_root=self.dirname, session_name=self.experiment_name,
                recording_name=recording_name, loadspikes=True, loadraw=False)
    ...
    def exportSpikes(self):
        ...
        self.convertSpikeData(
            self.OE_data.nwbData['acquisition']['\
                timeseries'][self.recording_name]['spikes'])
    ...
    def exportLFP(self, channel: int, lfp_type: str, gain: int):
        ...
        if self.settings.fpga_nodeId is None:
            self.settings.parse()
        fpgaNode = 'processor' + str(
            self.settings.fpga_nodeId) + '_' + str(self.settings.fpga_nodeId)
        data = self.OE_data.nwbData['acquisition']['\
            timeseries'][self.recording_name]['\
                continuous'][fpgaNode]['data'][:, channel]
    ...
    def convertPosData(self, xy: np.array, xy_ts: np.array) -> np.array:
        ...
        new_pos = np.c_[new_pos, np.ones_like(new_pos) * 1023, np.zeros_like(
            new_pos), np.zeros_like(new_pos)]
    ...
    def makeLFPData(
            self, hdf5_continuous_data: np.array, eeg_type='eeg', gain=5000):
        """
    ...
    def __filterLFP__(self, data: np.array, sample_rate: int):
        ...
        b = firwin(
            sample_rate+1, [lowcut, highcut], window='black', pass_zero=False)
