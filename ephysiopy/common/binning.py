...
class RateMap(object):
    ...
    def _getXYLimits(self):
        ...
        """
        x_lims = (np.nanmin(self.PosCalcs.xy[0]), np.nanmax(
            self.PosCalcs.xy[0]))
        y_lims = (np.nanmin(self.PosCalcs.xy[1]), np.nanmax(
            self.PosCalcs.xy[1]))
        self.x_lims = x_lims
        self.y_lims = y_lims
    ...
    def get_map(
        ...
        **kwargs,
        ...
        var_type : Variable2Bin
            The variable to bin. See ephysiopy.common.utils for legal values.

        map_type : MapType
            The kind of map returned. See ephysiopy.common.utils for legal values.
