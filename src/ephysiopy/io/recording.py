...
class OpenEphysBase(TrialInterface):
    ...
    def clusterData(self):
        if self.path2KiloSortData is not None:
            if self._clusterData is None:
                self._clusterData = KiloSortSession(self.path2KiloSortData)
                self._clusterData.load()
            return self._clusterData

    @clusterData.setter
    def clusterData(self, value):
        self._clusterData = value
    ...
    def get_available_clusters_channels(self) -> dict:
        ...
        K = KiloSortSession(self.path2KiloSortData)
        K.load()
